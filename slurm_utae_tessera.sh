#!/bin/bash
#
# 5-fold geographic CV for U-TAE + TESSERA.
# Submits one job per fold (array tasks 0–4).
#
# Usage:
#   sbatch --export=K=2,INPUT_YEARS=4 slurm_utae_tessera.sh
#   sbatch --export=K=1              slurm_utae_tessera.sh   # N=all
#   sbatch --export=K=2,TAG=slicing,FOLDS_FILE=src/data/geographic_folds.csv slurm_utae_tessera.sh
#
# The SLURM_ARRAY_TASK_ID is used as the --fold argument (0–4).
#
#SBATCH --job-name=utae_tessera
...
#SBATCH --mem=32G
#SBATCH --array=0-4
#SBATCH --output=logs/utae_tessera/fold%a_%A.out
#SBATCH --error=logs/utae_tessera/fold%a_%A.err

echo "=========================================="
echo "Starting U-TAE + TESSERA training job"
...
CMD="python train_utae_tessera.py --prediction_horizon $K --fold $FOLD"
